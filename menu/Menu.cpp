...
Menu::Menu(SDL_Surface *screen, std::vector<menuItem> *items) {
  ...
  this->currentItem = 0;
  TTF_Init();
  this->font = TTF_OpenFont("tata.ttf", 12);
...
void Menu::draw() {
  ...
  printf("Drawing menu... %d\n", i);
  while (i--) {
    SDL_SetColorKey(zozor, 0, SDL_MapRGB(zozor->format, 0, 0, 255));
    SDL_SetColorKey(bg, 0, SDL_MapRGB(bg->format, 0, 0, 255));
    SDL_FillRect(zozor, NULL, SDL_MapRGB(zozor->format, 255, 255, 0));
    SDL_FillRect(bg, NULL, SDL_MapRGB(bg->format, 255, 255, 0));
    text = TTF_RenderText_Blended(this->font, (*this->items)[this->items->size() - i - 1].text.c_str(), color);
